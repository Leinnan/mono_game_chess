...
namespace MonoGameAndroid1
{
    public sealed class GameState : State
    {
        ...
        public override void OnLoad(ContentManager content, GraphicsDevice graphics)
        {
            ...
            board.StartPos = new Vector2i((screenSize.Item1 - Consts.BOARD_SIZE * 80) / 2,
                (screenSize.Item2 - Consts.BOARD_SIZE * 80) / 2);
        ...
        public override void OnDraw(ref SpriteBatch spriteBatch)
        {
            ...
            {
                spriteBatch.Draw(fieldTexture,
                    new Rectangle(startPos.x + (moveablePiece.x * size.x), startPos.y + (moveablePiece.y * size.y),
                        size.x,
                        size.y), Color.Orange);
            }
            ...
            {
                spriteBatch.Draw(fieldTexture,
                    new Rectangle(startPos.x + (board.SelectedField.x * size.x),
                        startPos.y + (board.SelectedField.y * size.y), size.x,
                        size.y), Color.GreenYellow);
                foreach (var move in possibleMoves)
                {
                    spriteBatch.Draw(fieldTexture,
                        new Rectangle(startPos.x + (move.EndPos.x * size.x), startPos.y + (move.EndPos.y * size.y),
                            size.x,
            ...
            foreach (var piece in pieces)
            {
                spriteBatch.Draw(piece.isQueen ? pieceQueenTexture : pieceTexture,
                    new Rectangle(startPos.x + (piece.pos.x * size.x) + margin.x,
                        startPos.y + (piece.pos.y * size.y) + margin.y, size.x - margin.x * 2,
